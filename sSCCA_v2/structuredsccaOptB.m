function [myalpha, mybeta,myrho]= structuredsccaOptB(X,Y,mybetaold, myalphaold, tilderhoold,Taux,Tauy,edgesX,edgesY,weightsX,weightsY,mygamma,eta,method)
%this calls the CVX algorithm to estimate alpha and beta

[~, ~, ~,SigxxinvSigxy,SigyyinvSigyx]=mynonsccaOptB(X,Y);
...
edges2Y=edgesY(idy,:);
        ...
        cvx_begin quiet
                ...
                subject to
                norm( SigxxinvSigxy*mybetaold(:,1)-tilderhoold(1)*alphai,Inf)<=Taux;
          cvx_end
        ...
        cvx_begin quiet
                ...
                subject to
                norm( SigyyinvSigyx*myalphaold(:,1)-tilderhoold(1)*betai,Inf)<=Tauy;
       cvx_end
        %normalize alpha
