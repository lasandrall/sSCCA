function [maxcorr,hatalpha,hatbeta]= multistructuredsccaOptA(X,Y,ncancorr,TauX,TauY,edgesX,edgesY,weightsX,weightsY,method,mygamma,myeta,asnormalize);

%--------------------------------------------------------------------------
%multistructuredsccaOptA.m: function to estimate structured sparse CCA for 
%given tuning parameters using Option A in published manuscript. 
%Use this option if p/q is small to moderate (say p/q< 500)
%--------------------------------------------------------------------------

%If you want to select optimal tuning parameters, use
%cvmultistructuredsccaOptA.m

%USAGE
%[maxcorr,hatalpha,hatbeta]= multistructuredsccaOptA(X,Y,ncancorr,TauX,TauY,edgesX,edgesY,weightsX,weightsY,method,mygamma,myeta,asnormalize);
%[maxcorr,hatalpha,hatbeta]= multistructuredsccaOptA(X,Y,ncancorr,TauX,TauY,edgesX,edgesY,weightsX,weightsY,method);%defualt settings
...
%prepare inputs
[tildealpha, tildebeta, tilderho,~,~,~,~,~]=mynonsccaOptA(X,Y);
%initialize
tol=10^(-4);
...
while ( ((iter<maxiteration) && and( (diffalpha > tol) , (diffbeta >tol)) ) )
   ...
   [myalpha, mybeta,myrho]= structuredsccaOptA(X,Y,mybetaold, myalphaold,...
