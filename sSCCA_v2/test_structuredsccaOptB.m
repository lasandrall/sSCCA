...
%April 15, 2017
%structured sparse CCA examples for Option B
%load example data
clearvars; clc;
rng('default')
...
%data;
%cvout=cvmultistructuredsccaOptB(X,Y,ncancorr,edgesX,edgesY,weightsX,weightsY,method,mygamma,myeta,nfolds,ngrid);

%required parameters
method='Grouped' ; %'Fused' or 'Grouped';
ncancorr=1;
%will use default settings
cvout2=cvmultistructuredsccaOptB(X,Y,ncancorr,edgesX,edgesY,weightsX,weightsY,method);
...
TauX=cvout.optTauX;
TauY=cvout.optTauY;
[maxcorr,hatalpha,hatbeta]= multistructuredsccaOptB(Xtest,Ytest,ncancorr,TauX,TauY,edgesX,edgesY,weightsX,weightsY,method);
...
ncancorr=2;
%will use default settings
cvout=cvmultistructuredsccaOptB(X,Y,ncancorr,edgesX,edgesY,weightsX,weightsY,method);
%Alphaerr = norm(cvout.hatalpha * cvout.hatalpha'  - mytruealpha * mytruealpha', 'fro');
%Betaerr = norm(cvout.hatbeta * cvout.hatalpha'  - mytruealpha * mytruealpha', 'fro');
