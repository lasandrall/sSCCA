function cvout=cvmultistructuredsccaOptB(X,Y,ncancorr,edgesX,edgesY,weightsX,weightsY,method,mygamma,myeta,nfolds,ngrid,asnormalize);

%--------------------------------------------------------------------------
%cvmultistructuredsccaOptB.m: cross validation approach to
...
%multistructuredsccaOptB.m

%USAGE
%cvout=cvmultistructuredsccaOptB(X,Y,ncancorr,edgesX,edgesY,weightsX,weightsY,method,mygamma,myeta,nfolds,ngrid,asnormalize);
%cvout=cvmultistructuredsccaOptB(X,Y,ncancorr,edgesX,edgesY,weightsX,weightsY,method);%uses
%defualt settings
...
[Tauxrange, Tauyrange]= cvtunerangeOptB(X,Y,ncancorr,method);
...
for j=1:ncancorr;
    ...
    %         %Ynew
       ...
       while ( (iter<maxiteration) && (min(reldiff,max(diff_alpha,diff_beta))>tol)) %iterate if iteration is less than maxiteration or convergence not reached
            ...
            for jj=1:length(Tauxvec);
                 ...
                 Taux=(Tauxvec(:,jj));
                  for ii=1:nfolds
                    which=foldid==ii;
                    [tildeA, tildeB, tilderho,~,~]=mynonsccaOptB(Xold(~which,:),Yold(~which,:)); 
                    if(iter==1)
                       myalphaold=tildeA(:,j);
                       mybetaold=tildeB(:,j);
                       tilderhoold=tilderho(j);
                    end
                    [myhatalpha, myhatbeta,myrho]= structuredsccaOptB(X(~which,:),Y(~which,:),mybetaold, myalphaold, tilderhoold,Taux,Tauy,edgesX,edgesY,weightsX,weightsY,mygamma,myeta,method);
            ...
            [tildeA, tildeB, tilderho,~,~]=mynonsccaOptB(Xold,Yold);      
            ...
            [myalpha, mybeta,mytilderho]=structuredsccaOptB(X,Y,mybetaold, myalphaold,...
...
%              diff_beta=norm(abs(mybeta)-abs(mybetaold),Inf)
                ...
                diff_alpha=norm(myalpha-myalphaold,'fro')^2/norm(myalphaold,'fro')^2
                diff_beta=norm(mybeta-mybetaold,'fro')^2/norm(mybetaold,'fro')^2
                sumnew=norm(myalpha-myalphaold,'fro')^2 + norm(mybeta-mybetaold,'fro')^2;
                sumold=norm(myalphaold,'fro')^2+norm(mybetaold,'fro')^2;
